...
class SAC(object):
    def __init__(self, num_inputs, action_space, Config):
        ...
        self.critic = QNetwork(num_inputs, action_space, Config.hidden_size).to(device=self.device)
        self.critic_optim = Adam(self.critic.parameters(), lr=Config.lr)

        self.critic_target = QNetwork(num_inputs, action_space, Config.hidden_size).to(self.device)
        ...
        if self.policy_type == "Gaussian":
            ...
            self.policy = GaussianPolicy(num_inputs, action_space, Config.hidden_size, action_space).to(self.device)
            self.policy_optim = Adam(self.policy.parameters(), lr=Config.lr)

        else:
            self.alpha = 0
            self.automatic_entropy_tuning = False
            self.policy = DeterministicPolicy(num_inputs, action_space, Config.hidden_size, action_space).to(self.device)
            self.policy_optim = Adam(self.policy.parameters(), lr=Config.lr)
